...
- 2026-06-16 CT: Auth registry resolution must choose the server-only payload from `publishedEnvironments.test` when the browser `Origin` is `https://test.zoolandingpage.com.mx` or a configured `environmentAliases.test` host. Production origins continue to use `published`. This lets custom auth forms be tested against draft test registry changes without enabling them in production.
