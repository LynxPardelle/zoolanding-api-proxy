...
def payload(*, action="Modify", replacement="False"):
    return {
        "StackName": "example-test", "ChangeSetName": "release-1", "ChangeSetId": ARN,
        "Status": "CREATE_COMPLETE", "ExecutionStatus": "AVAILABLE",
        "Parameters": [{"ParameterKey": "EnvironmentName", "ParameterValue": "test"}, {"ParameterKey": "EnableFeature", "ParameterValue": "false"}],
        "Changes": [{"Type": "Resource", "ResourceChange": {"Action": action, "Replacement": replacement}}],
    }
...
class ReviewTestChangeSetTests(unittest.TestCase):
    ...
    def test_rejects_explicit_type_mismatch_or_invalid_expected_type(self):
        for response_type in (None, "", "CREATE", "IMPORT"):
            with self.subTest(response_type=response_type), self.assertRaisesRegex(ChangeSetReviewError, "change_set_identity_invalid"):
    ...
    def test_aws_description_still_requires_exact_identity_and_parameters(self):
        for field in ("StackName", "ChangeSetName", "ChangeSetId"):
            with self.subTest(field=field), self.assertRaisesRegex(ChangeSetReviewError, "change_set_identity_invalid"):
    ...
    def test_accepts_exact_aws_noop_without_change_set_type(self):
        candidate = {**payload(), "Status": "FAILED", "ExecutionStatus": "UNAVAILABLE", "Changes": [],
